...
if ((isset($_SESSION['id']) || isset($_GET['edit'])) && !isset($_POST['update'])) {
    ...
    $required_fields = array('Username', 'Email', 'Kasi', 'Cell', 'Country', 'Province', 'City', 'Address', 'Zip');
    ...
    if (empty($form_errors)) {
        try {
            ...
            $update_customer = "UPDATE customers SET c_username=:username,c_email=:email,c_kasi=:kasi,c_contact=:cell,c_country=:country,c_province=:province,c_city=:city,c_street_address=:address,c_zip=:zip WHERE id=:id";

            // use PDO to prepare and sanitize the data
            $statement = $db->prepare($update_customer);

            // Add the data into the database 
            $statement->execute(array(':username' => $username, ':email' => $email, ':kasi' => $kasi, ':cell' => $contact, ':country' => $country, ':province' => $province, ':city' => $city, ':address' => $address, ':zip' => $zip, ':id' => $hidden_id));
