...
$sqlCheckCart = "SELECT * FROM cart WHERE ip_address=:ip_address";
$statementCheckCart = $db->prepare($sqlCheckCart);
$statementCheckCart->execute(array(':ip_address' => $ip_add));

$statementCheckAmount = $db->prepare($sqlCheckCart);
$statementCheckAmount->execute(array(':ip_address' => $ip_add));


while ($rs = $statementCheckAmount->fetch()) {
    $cart_id = $rs['cart_id'];
    $shop_id = $rs['m_id'];
